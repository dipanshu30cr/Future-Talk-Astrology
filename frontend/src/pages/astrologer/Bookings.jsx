...
const Bookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        setLoading(true)
        const response = await axios.get("http://localhost:9000/api/bookings/astrologer/")
  ...
  const handleUpdateStatus = async (bookingId, newStatus) => {
    try {
      await axios.put(`http://localhost:9000/api/bookings/astrologer/${bookingId}/`, {
        status: newStatus,
      })
  ...
  const filteredBookings = bookings.filter((booking) => {
    const matchesSearch =
      booking.customer_details.first_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      booking.customer_details.last_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            {currentBookings.map((booking) => (
              <BookingCard key={booking.id}>
                ...
                <BookingInfo>
                  ...
                  <BookingActions>
                    <ActionButton to={`/astrologer/bookings/${booking.id}`}>View Details</ActionButton>
                    ...
                    {booking.status === "PENDING" && (
                      <StatusButton primary onClick={() => handleUpdateStatus(booking.id, "CONFIRMED")}>
                        Confirm Booking
                      </StatusButton>
                    )}

                    {booking.status === "CONFIRMED" && (
                      <StatusButton primary onClick={() => handleUpdateStatus(booking.id, "COMPLETED")}>
                        Mark as Completed
                      </StatusButton>
                    )}
                  </BookingActions>
